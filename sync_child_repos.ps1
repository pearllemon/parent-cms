...
if ($RepoName) {
    Write-Log "Targeting single repository: $RepoName"
}
...
foreach ($repoName in $ChildRepos) {
    ...
    # Synchronize WordPress Plugins (Elementor, Elementor Pro)
    $srcPluginsDir = "F:/MCP For Pearl Lemon/Lovable Project/Plugins"
    $destPluginsDir = Join-Path $repoPath "Plugins"
    if (Test-Path $srcPluginsDir) {
        Write-Log "Syncing WordPress Plugins..."
        if (-Not (Test-Path $destPluginsDir)) {
            New-Item -ItemType Directory -Path $destPluginsDir | Out-Null
        }
        $robocopyArgs = @(
            "$srcPluginsDir",
            "$destPluginsDir",
...
"@
    ...
    if ($repoName -eq "breeamassessment") {
        Write-Log "Copying BREEAM-specific homepage seed migration..."
        ...
        Copy-Item -Path (Join-Path $ParentPath "supabase/breeam_homepage_seed.sql") -Destination $migrationPath -Force
    }
    ...
    if ($status) {
        ...
        # Optimize git add to only add the specific CMS folders/files we sync
        git add src/ supabase/ package.json vite-plugin-cms-snapshot.ts vite.admin.config.ts 2>&1 | ForEach-Object { Write-Log $_ }
        # If there are changes in Plugins, add them specifically to avoid scanning them every time
        if (git status --porcelain | Select-String "Plugins/") {
            Write-Log "Adding Plugins directory changes..."
            git add Plugins/ 2>&1 | ForEach-Object { Write-Log $_ }
        }
        git commit -m "Sync CMS from parent (non-destructive)" 2>&1 | ForEach-Object { Write-Log $_ }
